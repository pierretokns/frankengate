...
func TestRDBReservationLifecycleIsIdempotentAndFenced(t *testing.T) {
	...
	renewed, err := store.Renew(ctx, reservations.RenewRequest{ReservationID: first.ID, AttemptEpoch: 3, LeaseUntil: now.Add(2 * time.Minute), Now: now})
	require.NoError(t, err)
	require.WithinDuration(t, now, renewed.UpdatedAt, time.Second)
