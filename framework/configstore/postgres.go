...
func newPostgresConfigStore(ctx context.Context, config *PostgresConfig, logger schemas.Logger) (ConfigStore, error) {
	...
	d.db.Store(db)
	notifyConfig, err := pgx.ParseConfig(dsn)
	if err != nil {
		postgresconn.Close(db, logger)
		return nil, fmt.Errorf("parse postgres notification connection config: %w", err)
	}
	d.virtualKeyInvalidationNotifyDial = func(ctx context.Context) (virtualKeyInvalidationNotifyConn, error) {
