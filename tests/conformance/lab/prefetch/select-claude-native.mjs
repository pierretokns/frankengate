...
const [root, version, arch, expectedName, expectedTarball, expectedIntegrity] = process.argv.slice(2);
...
if (name !== expectedName || !expectedTarball?.startsWith("https://registry.npmjs.org/") || !expectedIntegrity?.startsWith("sha512-")) throw new Error("source-lock native identity mismatch");
...
if (wrapper.version !== version || wrapper.optionalDependencies?.[name] !== version || installed.name !== name || installed.version !== version || locked?.version !== version || locked.integrity !== expectedIntegrity || locked.resolved !== expectedTarball) throw new Error("native package coordinate/integrity mismatch");
