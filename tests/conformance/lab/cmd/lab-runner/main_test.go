...
func TestSanitizedFailureClassificationMutants(t *testing.T) {
	cases := []struct {
		...
		{"SQLSTATE 42703", false, "postgres-schema"},
		{"permission denied for table config", false, "postgres-permission"},
		{"SQLSTATE 23505", false, "postgres-constraint"},
		{"migration failed at revision 4", false, "postgres-migration"},
		{"APP_DIR is not writable", false, "app-dir-permission"},
		{"failed to initialize config store", false, "config-store-bootstrap"},
