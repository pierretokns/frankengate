...
var (
	...
	immutableGitRef = regexp.MustCompile(`^git:[0-9a-f]{40}$`)
)
...
func DecodeRuntimeLock(reader io.Reader) (*RuntimeLock, error) {
	...
	if err != nil || len(data) > 1<<20 {
		return nil, fmt.Errorf("runtime lock exceeds the bounded input contract")
	}
	if err := rejectDuplicateJSONKeys(data); err != nil {
		return nil, fmt.Errorf("runtime lock JSON: %w", err)
	}
...
func (lock RuntimeLock) Validate() error {
	if !runtimeRunID.MatchString(lock.RunID) || !sha256Value.MatchString(lock.SourceLockSHA256) {
		return fmt.Errorf("incomplete runtime lock identity")
	}
	want := []string{"bifrost", "claude-runner", "codex-runner", "egress-sentinel"}
	switch lock.Schema {
	case RuntimeLockSchema:
		if lock.RecorderPolicySHA256 != "" || len(lock.Images) != 4 {
			return fmt.Errorf("incomplete %s lock", RuntimeLockSchema)
		}
	case RuntimeLockSchemaV2:
		if !sha256Value.MatchString(lock.RecorderPolicySHA256) || len(lock.Images) != 5 {
...
func (lock RuntimeLock) IsRecorderCapable() bool {
	return lock.Schema == RuntimeLockSchemaV2 && lock.Validate() == nil
}

func (lock RuntimeLock) NetworkRecorderImage() (RuntimeImage, bool) {
...
func DecodeLock(reader io.Reader) (*Lock, error) {
	data, err := io.ReadAll(io.LimitReader(reader, (1<<20)+1))
	if err != nil || len(data) > 1<<20 {
		return nil, fmt.Errorf("image lock exceeds the bounded input contract")
	}
	if err := rejectDuplicateJSONKeys(data); err != nil {
		return nil, fmt.Errorf("image lock JSON: %w", err)
	}
	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	var lock Lock
