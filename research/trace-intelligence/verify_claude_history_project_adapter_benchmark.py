#!/usr/bin/env python3
"""Verify the content-minimized Claude project-adapter receipt."""
...
def verify(input_path: Path, output_path: Path) -> dict[str, object]:
    ...
    checks = {
        ...
        "aggregate_modes_present": set(result.get("aggregate", {})) == {"prompt", "all"},
        "nonempty_sessions": result.get("source", {}).get("session_count", 0) > 0,
    }
    receipt = {"schema_version": "frankengate-claude-history-project-adapter-verification-v1", "passed": all(checks.values()), "checks": checks, "result_sha256": expected}
