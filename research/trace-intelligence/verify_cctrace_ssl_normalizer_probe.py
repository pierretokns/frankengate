#!/usr/bin/env python3
"""Verify the content-minimized real-session normalization receipt."""
...
def verify(result_path: Path) -> dict[str, Any]:
    ...
    if result.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("unexpected cctrace SSL schema")
    ...
    rates = ("tool_names_exact_order_rate", "logical_action_count_exact_rate", "logical_orders_exact_rate", "logical_resources_exact_order_rate", "identifier_fidelity_rate", "all_evidence_grounded_rate", "evidence_grounding_rate")
    for field in rates:
    ...
    for record in records:
        if record.get("status") == "ok":
            ...
            for field in ("tool_names_exact_order", "logical_action_count_exact", "logical_orders_exact", "logical_resources_exact_order", "identifier_fidelity", "all_evidence_grounded"):
                if not isinstance(record.get(field), bool):
                    raise ValueError(f"missing per-record boolean {field}")
        elif record.get("status") != "error":
            raise ValueError("unknown record status")
    return {"schema_version": "frankengate-cctrace-ssl-normalizer-verification-v1", "source_result_sha256": file_hash(result_path), "selected_verified": selected, "completed_verified": completed, "failures_verified": failures, "claim_boundary_verified": True, "verification_passed": True}
