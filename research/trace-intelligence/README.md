...
The new [`trace-mined BIRD frontier factorial`](experiments/summaries/bird-sql-trace-mined-factorial-20-2026-08-08.md)
uses only independently validated SQL found in recorded tool calls; it scored
...
Tool-class stratification shows the prior is not uniform: exact same-project
...
The [`term-context collision diagnostic`](experiments/summaries/claude-history-term-context-collisions-2026-08-09.md)
then found 543/778 shared terms with at least one project pair below `.05`
lexical-context Jaccard, providing a cheap hard-negative feature before
identifier, temporal, or human review.
...
enterprise transfer. A hash-only recurrence screen found 9 successful
multi-session candidates (2 spanning multiple project labels and 4 model
...
and its nested span tree resolves 810/812 aligned annotation locations; it has
no principal/tenant identity or independent task outcome. The acquired
...
The paired five-arm preflight is frozen in
[`recovery-bench-intervention-matrix-2026-08-05.json`](experiments/manifests/recovery-bench-intervention-matrix-2026-08-05.json);
it verifies task-set identity before any Harbor/model run and makes no outcome
claim.
