...
| Strict tool-artifact recurrence in one long history | 70,969 paired/near-paired tool uses yielded 2,012 recurring strict identities, 1,105 cross-project, 431 mixed-outcome identities, and 3,866 error→success recoveries | Real traces can seed a governed artifact/recovery queue; recurrence and tool success are not correctness or skill labels |
| Temporal prior-success prediction | No-prior tool success was `88.7216%`; same-project prior success `96.8268%`; other-project prior success `97.1615%`, with session-boundary leakage prevention | Prior success is a useful candidate-prior feature, but observed tool success is not semantic correctness or causal skill utility |
| Parameterized key-shape control | Input-key-only prior success (`90.5514%` same-project, `90.7613%` other-project) fell below no-prior key-shape success (`92.3108%`) | Coarse templates are recall candidates only; exact bindings/resource identity and replay are required |
| Tool-class prior stratification | Exact same-project priors lifted shell success `.697511→.938326` and mutation `.941272→.993243`; read/search was already `.993934` | Prior-based reuse needs tool-class-specific safety/risk gates, not one aggregate policy |
| Error→success recovery mining | 3,866/4,506 same-session retries after an earlier identity error succeeded; shell recovery was `.335766`, mutation `.762500`, read/search `.941319` | Recovery traces are targeted eval/friction candidates, not artifact or skill promotion labels |
| Same-user scoped artifact support | Peter yielded 3,158 candidates repeated across sessions, 518 across projects, and 460 cross-project/friction-adjacent candidates; repeat-friction rate `.480813` versus `.436807` for single-session candidates, while Vaynelee reversed direction | A scoped personal/project library is the first positive reuse lane; recurrence is not a universal friction or correctness signal |
| Project-held-out lexical adaptation | Full Peter cohort combined prompt+tool MRR `.769341→.854452`; Vaynelee combined MRR stayed `.978495` at ceiling; tool-only Peter MRR slightly declined. A separate 404-session Claude-history proxy improved user-message MRR `.885892→.915765` across 37 project folds. | Scoped adaptation is repeatable when the baseline has headroom, but project labels are silver and no universal custom embedding or artifact-utility promotion follows |
...
6. **Dataset fit is part of the method.** Even deliberately schema-agnostic
   ...
   labels.
7. **Cross-user overlap is not cross-user reuse.** Loose normalized tool forms
...
9. **Adaptation needs a saturation check.** The project-held-out adapter helped
   the larger Peter sample but had no combined lift in the near-ceiling
   Vaynelee sample, and tool-only features were inconsistent. Any custom
   embedding or adapter gate needs absolute headroom, project/time holdouts,
...
- [Claude-history project adapter study](../experiments/summaries/claude-history-project-adapter-2026-08-09.md)
- [Claude-history tool-artifact miner](../experiments/summaries/claude-history-tool-artifact-miner-2026-08-09.md)
- [Claude-history temporal tool-artifact benchmark](../experiments/summaries/claude-history-tool-artifact-temporal-2026-08-09.md)
