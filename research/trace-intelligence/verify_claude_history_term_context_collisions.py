#!/usr/bin/env python3
"""Verify the content-minimized term-context collision receipt."""
...
def verify(input_path: Path, output_path: Path) -> dict[str, object]:
    ...
    checks = {
        ...
        "protocol_present": isinstance(result.get("protocol"), dict),
        "nonempty_context_pairs": result.get("coverage", {}).get("context_pair_count", 0) > 0,
    }
    receipt = {"schema_version": "frankengate-claude-history-term-context-collisions-verification-v1", "passed": all(checks.values()), "checks": checks, "result_sha256": expected}
