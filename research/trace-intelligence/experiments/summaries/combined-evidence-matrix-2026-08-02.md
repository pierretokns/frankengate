...
Real otel conformance, governed history mechanics, synthetic memory invariants, and one loss-aware real memory transition cohort pass; silver-label retrieval, same-candidate local postgresql rls, and governed skill-release mechanics pass partially; memory utility, diagnosis, causal skill benefit, cross-user learning, aurora scale, and prospective enterprise utility do not.
...
- The gated enterprise analysis layer exercised 4 answer shapes (similar work, friction/recovery, reviewed skill gaps, and opt-in collaboration) on a content-free cohort. Its mechanics passed=True; this proves safe computation after authorization, not validity or utility of any enterprise conclusion.
...
- The explicit namespaced capability extension completed profile-aware round trips for Wisp=True and MATM=True; aggregate structural retention was 0.917 and 0.878. It preserves authority/epoch, reset/termination, reward, replay references, and memory lineage only for profile-aware readers; omitted payload/state manifests remain non-replay evidence.
- Frontier Luna's independent schema review returned `revise`; 4 boundary changes were applied. This is a structural review receipt, not a semantic utility judgment.
...
- A failure-bearing real BIRD-SQL SkillGen reproduction gave the frontier adapter 2/8 training successes and generated a candidate from 6 failures. On 8 held-out questions, exact SQLite replay moved from 0.500 to 0.375: repairs=0, regressions=1, net gain=-1. The release gate passed=False; this is direct negative cohort-specific efficacy evidence, not a reason to discard the independent replay and proposal mechanics.
- An independent upstream RHO round accepted its harness edit by self-preference (mean=1.000), but matched no-harness LOCOMO replay on 2 held-out questions moved from 0.703 to 0.511 (delta=-0.192; 1 regressions). RHO's self-preference signal therefore did not predict independent utility on this slice and the candidate is not eligible for promotion.
- The pinned upstream ReasoningBank LOCOMO attempt reached local embedding setup but stopped at its memory judge because the documented Azure Foundry path requires the unavailable `az` executable. It is typed provider-unavailable evidence, not a memory-quality or utility result.
- With the upstream runner preserved and only the memory client substituted, ReasoningBank + Codex scored 0.593 versus the matched no-harness 0.703 on two held-out LOCOMO questions (delta=-0.110; 1 regression). This bounded negative slice does not authorize memory promotion.
...
- The MATM outcome-conditioned successful-neighbor arm covered 2130 rows across 34 held-out model folds. Its top-10 recommendation rate was 0.747 versus 0.681 for all-trace neighbors (lift 0.067), but mean AUC fell by 0.056; the bootstrap intervals cross zero, so this is promising recommendation signal rather than causal skill evidence.
- The environment-backed ALFWorld pilot ran 18 episodes over 2 held-out tasks. The expert control won 2 tasks, while the trace-derived procedure won 0; this is a small negative pilot and does not authorize promotion.
- The revised ALFWorld contract replay added 6 diagnostic episodes (0 candidate wins, 56 candidate invalid actions), then added a four-family held-out replay of 16 episodes. The r3 candidate won 0 tasks and emitted 168 invalid actions; causal skill benefit remains false.
- The Qwen3 4B cross-model probe added 8 episodes on the same four paths. Its candidate won 0 and emitted 36 invalid actions; the 12-step cap makes semantic quality comparison invalid, so this remains harness evidence rather than skill benefit.
- An independent projection verifier matched the committed r3/r4 aggregates to the external raw run summaries (True); it verifies receipt integrity and claim boundaries, not causal task success.
...
- The larger r9 family-disjoint replay added 8 previously unused tasks. Llama 3.2's trace-mined arm won 0 and emitted 206 invalid actions; causal benefit remains False. The identical Qwen replay was recorded as incomplete runtime evidence: True. The r9 aggregate verifier passed: True. The second-harness r11 replay also produced 0 wins and 66 invalid actions, with verifier True. The replayable r12 cohort independently recomputed 32 rows with zero mismatches: True; all executed actions were admissible: True. The r13 control arm added ['no_skill', 'formatting_placebo', 'trace_mined_procedure_v2'] across 8 tasks, independently verifying 48 rows: True; no arm produced a win.
- The r14 Qwen 3 4B native-harness control replication covered 8 tasks and ['no_skill', 'formatting_placebo', 'trace_mined_procedure_v2'] arms; its fresh verifier checked 24 rows with pass=True. All three arms remained at 0/8 wins.
...
- A query-independent digest-backed procedure was actually released from pre-cutoff natural evidence across 3 anonymous projects and 23 queries. Its independent verifier passed for 3 projects and it was visible on 23 queries, but matched 0 target states. This closes the natural release-lineage gate while leaving procedure quality and utility unproven.
- The content-free model-generated natural-procedure comparison covered three projects per model: Qwen3 4B passed 0/3 structural gates, Llama 3.2 passed 1/3, and frontier Luna passed 1/3. All independent receipt verifiers passed. This measures format/grounding mechanics only; semantic procedure utility and changed-system outcomes remain unmeasured.
- A corrected frontier-only Luna sample admitted 4 public sessions with at least 20 tool calls and one recovery candidate. It produced valid JSON for 4/4 and passed the controlled grounding/shape rubric for 3/4; the independent verifier passed=True. This corrects fixture-selection bias but remains structural evidence, not procedure utility or causal skill benefit.
- The frontier Luna SkillOpt checkpoint replication covered 4 previously unused tasks across 4 families and 12 total arm episodes. No-skill, placebo, and the published SkillOpt checkpoint each won zero tasks; the fresh verifier checked 12 rows with pass=True. The 12-step horizon truncated all arms, so this is stronger model/family protocol evidence but not a general semantic skill estimate or promotion result. It consumed 144 frontier calls; mean arm wall times were {'no_skill': 78415.235, 'formatting_placebo': 87050.662, 'skillopt_candidate': 103005.202} ms, which is a measured operating-cost signal rather than a quality result.
- The fair-horizon Luna follow-up extended one held-out task to 35 steps. No-skill, placebo, and SkillOpt each remained at zero wins; the independent verifier checked 3 rows with pass=True. This removes the short-horizon explanation for that task while remaining a one-task model/harness slice, not a general skill or enterprise-outcome estimate.
...
| --- | --- | --- |
| Show each user all history | `local_mechanics_supported` | complete permission-oracle/deletion/failover gauntlet |
...
| Similar work across users | `analysis_mechanics_only` | authorized SWE-chat or consented enterprise same-task labels with cohort privacy |
| Missing cloud/domain skills | `analysis_mechanics_only` | reviewed capability taxonomy, environmental-availability labels, abstention, and prospective task uplift |
| Who should collaborate | `analysis_mechanics_only` | reciprocal opt-in introductions and independently measured outcomes under minimum cohorts |
