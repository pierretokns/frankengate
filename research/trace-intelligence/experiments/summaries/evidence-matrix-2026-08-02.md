...
| A frontier agent can consume a validated SQL artifact under governance | One paired Defog paraphrase task: no-skill, placebo, and validated-artifact arms all reached semantic correctness; the artifact arm was independently recomputed with zero authority or verifier errors. | **Bounded consumption mechanics** | Artifact execution and submission can be safely wired into the governed loop. | Train-only artifact library, unpaired retrieval, NIL candidates, larger changed-system replay, and reuse-versus-regeneration lift. |
