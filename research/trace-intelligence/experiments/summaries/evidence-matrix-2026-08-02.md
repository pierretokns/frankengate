...
| Nearest-question retrieval of validated SQL transfers to held-out tasks | Ten broker/car-dealership `questions_gen` targets retrieved executable same-database artifacts from basic/advanced source tasks; all 10 executed under authority, but 0/10 matched target semantics. | **Negative bounded retrieval result** | “Store successful SQL and retrieve the nearest question” is insufficient; validated execution does not imply relevance. | Exact/structured+dense/frontier comparison, parameterized templates, NIL/wrong-system labels, regeneration control, and changed-schema replay. |
...
| Domain adaptation automatically improves embeddings | MATM fold-local adapter: Recall@20 .5301→.5331, delta +.0029, CI crosses zero; MRR slightly decreased. A separate 601-case, four-family-held-out schema-grounded benchmark found frozen Nomic MRR `.1824` / R@1 `.0933` / R@10 `.3760` versus schema-adaptive pair scorer `.1573` / `.0761` / `.3214`; same-scope collision-before-target was `.1013` versus `.1094`. | **Null / not promotable** | “Train an adapter” is not evidence of useful adaptation; generated schema labels and hard negatives did not beat the frozen baseline here. | Corporate hard negatives, entity/time/project holdouts, SME alias/NIL labels, and a preregistered downstream utility gate. |
