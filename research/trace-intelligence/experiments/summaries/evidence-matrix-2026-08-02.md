...
| A Trace2Skill-style compiler can transfer a procedure | One compiled native pilot scored 4/4 vs 3/4 controls; sequential prefix transfer scored 2/2, tying no-skill and beating neutral once; the complete car→broker replication executed safely but tied no-skill and neutral while using 8 vs 6 SQL attempts. | **Transferability without utility** | Compilation, hashing, isolation, and replay are feasible; a compiled artifact can survive a family change. | Demonstrate incremental lift over no-skill and neutral on a larger sequential cohort, including repair/regression and efficiency. |
