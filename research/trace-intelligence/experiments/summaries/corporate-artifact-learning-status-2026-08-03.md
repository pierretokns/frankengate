...
| Unpaired validated-artifact retrieval | Ten held-out broker/car tasks retrieved validated same-database SQL artifacts via lexical overlap; all executed under authority, but semantic reuse was 0/10. | **Negative bounded retrieval result** | Nearest-question lookup cannot be the artifact library's sole selection strategy. | Structured/dense/frontier retrieval, parameterized templates, NIL labels, regeneration and changed-system utility. |
...
| Domain-specific embedding adaptation | Database-family-held-out MATM adapter underperformed deterministic structured scoring and reached 51.2% collision-before-target. A separate 2026 schema-retrieval study reports strong leave-one-corpus-out gains from schema-generated queries plus granularity-aware hard negatives. | **Negative on proxy; schema recipe still open** | Re-run on Defog/BIRD schema-grounded positives and hard negatives, then require structured-retrieval and changed-system utility lift. |
