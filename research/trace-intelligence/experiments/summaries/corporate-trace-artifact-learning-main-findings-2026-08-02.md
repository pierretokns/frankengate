...
| Frontier ontology proposal/population is mechanically feasible but not self-validating | On two public Fable trace excerpts, both arms returned valid structured output for `2/2`; GOI-style proposal averaged 16 entities/7.5 relations with evidence grounding `.889`, while starter-schema population averaged 11/4.5 with grounding `.900` | A starter schema reduces free-form expansion and slightly improves evidence grounding in this tiny proxy; no ontology, alias, authority, temporal, or replay quality is established |
