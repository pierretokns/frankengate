...
| Recorded schema-first projections add a strong structured signal | On 72 held-out WMH-BIRD tasks, lexical A0 reached MRR `.8050` / R@1 `.7083`; table-and-column identifiers parsed only from recorded tool DDL reached `.9595` / `.9306`; provenance/alias edges alone reached `.8472` | Schema-first typed projections are promising for structured retrieval, but the labels are SQL table references rather than reviewed corporate concepts |
| Authority filtering is a useful fail-closed mechanic | A synthetic fault fixture injected 68 unauthorized edges; unconstrained ranking selected 16 unsafe top-1 candidates, while filtering selected zero and raised target R@1 `.7353 -> .8676` | Keep policy filtering before ranking/promotion; this is not evidence of real RLS or policy-epoch correctness without principal-labelled traces |
...
| The ontology/dense/frontier arms now have one aligned public cohort | On the frozen 44-case cohort, schema-first A1 reached MRR `.9489` / R@1 `.9091`, dense `.9402/.9091`, and frontier `.9545/.9091`; frontier's replay-compatible selection was `.9280` versus dense `.4082` | Typed identifiers, dense recall, and frontier compression can be compared without cohort drift; these remain public-proxy measurements |
| A same-run typed → frontier cascade gives a small measurable lift | On the same 44-case cohort, schema-first → Luna reached MRR `.9773` / R@1 `.9545` versus full-pool Luna `.9659/.9318`, while selecting 2.045 versus 2.091 tables and matching compatible selection `.9167` and invalid selections `.25` | Cheap recorded schema structure can remove distractors before frontier review; this is an implementation signal, not evidence of reviewed corporate ontology quality or enterprise skill transfer |
| Retrieval and frontier judgment still lack enterprise-semantic validation | The aligned sequential run had 88/88 valid calls and zero failures, but its labels are recorded SQL table references on SQLite replay with no human aliases, NIL labels, principals, epochs, or changed-system outcomes | Keep the cascade behind replay, authority, and human review gates; the next test must use a consented semantic enterprise cohort |
| EnterpriseRAG-Bench has the right document-side slices but not trace-side evidence | A direct audit of the public question parquet found 500 questions, 2,427 atomic facts, 742 expected-document references, 93 multi-document cases, and explicit conflict/completeness/abstention/constrained categories; no principals, tool outcomes, exposure events, or authority epochs | Use it as a document retrieval and hard-negative fixture; never treat its score as proof of ontology induction, skill learning, or cross-user enterprise utility |
| Lexical retrieval fails exactly where semantic and abstention controls matter | Full-corpus SQLite FTS5 over 511,962 documents reached target-bearing MRR `.4459`, R@1 `.3787`, R@10 `.5787`; semantic R@1 `.08`; all 30 targetless questions still returned non-empty results | Add semantic candidate recall and explicit abstention/authority gates; a non-empty retrieval result is not evidence that an artifact or skill should be promoted |
| EnterpriseRAG-Bench's difficulty is strongly slice-dependent | In the lexical receipt, semantic MRR/R@10 were `.1137/.208`, completeness `.5954/.75`, conflicting-info `.8042/.90`, and project-related `.6734/.90`; all 30 high-level/info-not-found questions received non-empty results despite having no gold document IDs | Keep semantic, completeness, conflict, project, and NIL/abstention metrics separate; an overall RAG score hides the exact failure modes relevant to corporate trace mining |
| Public enterprise tuning practice is staged, not raw-log pretraining | Databricks, Google, AWS, and Cohere documentation converges on metadata/retrieval baselines, labeled query–corpus examples or hard negatives, held-out evaluation, and versioned deployment; Databricks explicitly advises diagnosing the embedding gap before fine-tuning | Mine traces into labeled examples and reusable artifacts first; treat custom embeddings/rerankers as optional compressed components with rollback, not as ontology truth |
| Frontier review rescues semantic ranking but cannot fix candidate recall | On all 125 EnterpriseRAG semantic questions, Luna reranking lexical top-20 improved MRR `.1137 -> .1947` and R@1 `.08 -> .184`; the top-20 pool contained a target only `.224` of the time and R@10 stayed `.208` | Improve the candidate generator (dense, alias, identifier, metadata) before adding more model review; report pool recall and rerank gain separately |
| A generic dense model is worse than lexical retrieval on the enterprise-shaped semantic slice | Full-corpus MiniLM reached R@20 `.12` with title/snippet representation and `.064` title-only, versus lexical top-20 `.224` | Do not assume embeddings solve corporate aliases; test hard-negative/domain adaptation and stronger model families before adding vector infrastructure |
...
| Exposure-aware trajectory supervision is not automatically better than lexical search | On 10 LRAT trajectories, a leave-one-trajectory-out TF–IDF/logistic ranker trained on browsed versus exposed-but-unbrowsed documents reached MRR `.5945`, R@1 `.50`, and R@10 `.70`, below lexical `.7667/.60/1.00` | Later browsing is a valid candidate supervision signal, but silver behavior labels can encode exploration or confirmation rather than correctness; require adjudicated relevance and downstream replay before distillation |
...
| LaBSE produces broader Oracle-inequality candidate coverage than the paper composite on TechQA | On the 500-page/100-query fixture, LaBSE selected `56/100` inequality-valid negatives versus `23/100` for the six-encoder composite; the same reranker reached MRR@10 `.6936` versus `.6950` | This is coverage, not quality. An annotated gold-page audit found `0/56` and `0/23` explicit false negatives, but unmarked pages still require blinded adjudication; retain LaBSE as a diversity/control arm rather than promote it |
| LaBSE's apparent advantage is not explained by published gold-page contamination | Neither LaBSE nor the composite selected a page listed as another `gold_page_id` on the same fixture | The audit is weak because gold labels are incomplete; measure human/independent relevance, identifier collisions, and downstream reranker utility before drawing a quality conclusion |
| Frontier adjudication separates LaBSE coverage from hard-negative quality | In 12 candidates per arm with two Luna calls, the composite received `19/24` near-miss labels and `5/24` unrelated; LaBSE received `9/24` near-miss and `15/24` unrelated; judged relevant false negatives were `0/24` for both and repeat agreement was `9/12` per arm | On this bounded public sample, the composite is the stronger contrastive-negative source and LaBSE is primarily a diversity/control arm; labels remain silver until SME review and outcome replay |
| Same-query fair controls reveal a quality/safety tradeoff | With one deterministic query cohort across all arms, composite was `12/24` near-miss / `4/24` judged relevant false negatives; LaBSE `8/24` / `3/24`; lexical `14/24` / `3/24`; random `2/24` / `0/24` | Composite and lexical produce useful hard negatives but can contaminate training; LaBSE is not automatically safe. Require SME false-negative labels plus downstream reranker/replay utility before selection-arm promotion |
| ComRAG-style centroid memory trades recurrence for bounded storage | On 40 chronological public trace sessions, centroid memory matched full-history proxy hits at 11–12 clusters; at four clusters it fell from shape/project `.333/.667` to `.250/.583`; high/low quality routing added no lift | Centroid consolidation is a viable bounded memory mechanism, not a free replacement for immutable artifacts. Test stale/conflict/epoch mutations, dense embeddings, and validated outcomes before promotion |
...
| New ontology generators improve structure, not enterprise truth | GOI reports strong structural coverage in controlled ontology-shaped evaluations; OntoGPT/SPIRES provides schema-grounded extraction and population, but assumes a starter ontology/template | Use GOI as a typed proposal arm and OntoGPT as a population arm; retain identity, temporal, authority, NIL, evidence, and replay gates, and never promote one-shot graph output |
| Frontier ontology proposal/population is mechanically feasible but not self-validating | On five public Fable trace excerpts, both arms returned valid structured output for `5/5`; GOI-style proposal averaged 14.2 entities/7.0 relations with evidence grounding `.952`, while starter-schema population averaged 7.0/4.2 with grounding `.781` | A starter schema reduces free-form expansion, but its grounding advantage did not replicate; no ontology, alias, authority, temporal, or replay quality is established |
| Frontier ontology output has measurable run variance | An identical five-case repeat changed GOI mean entities/relations `14.2/7.0 → 10.2/4.8` while grounding stayed `.952→.968`; starter-schema output stayed `7.0/4.2 → 7.2/4.2` with grounding `.781→.775` | Treat starter schemas as a structural-stability aid, not a correctness guarantee; require canonical identity labels and adjudicated edge agreement before promotion |
| A second frontier pass exposes unsupported-edge and review burden risk | On the five-case repeat, a separate Luna judge marked GOI items supported `.867`, unsupported `.013`, unclear `.120`; starter-schema items supported `.807`, unsupported `.035`, unclear `.158` | Use model judgment only to prioritize review; these are silver labels from the same model family, not ontology precision or a release gate |
