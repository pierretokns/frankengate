...
| Validated subplans can help a frontier agent compose a new answer | Two seeded replays on five source-disjoint broker tasks: composable trace-mined procedure `10/10` semantic correctness versus no-skill `7/10` and formatting placebo `6/10`; independent verification passed, with zero unauthorized observations and fewer SQL/tool calls | Promising composability signal after the whole-query coverage null; not a powered causal, cross-family, or enterprise result. Test parameterized artifacts, regeneration, changed schemas, and negative transfer next |
