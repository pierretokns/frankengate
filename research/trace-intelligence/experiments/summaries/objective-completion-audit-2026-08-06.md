...
| Cross-user insights | Trace Commons prompt retrieval reached `13/13` and durable-identifier retrieval `12/13` same-project top-1 on a repeated-workstream proxy; event structure was `1/13`. The corrected local Codex archive importer exposes 622 sessions and 47,122 prompt episodes, but no stable enterprise labels or independent outcomes. | Workstream/friction-signal proxies only; cross-user insight unproven | Consent-stable identities, principal/time-disjoint task labels, human capability labels, same-surface negatives, and negative-transfer gates. |
