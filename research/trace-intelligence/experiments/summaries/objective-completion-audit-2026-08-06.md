...
|---|---|---|---|
| Reusable validated SQL/tool artifacts | On recorded BIRD traces, only `76/193` executable tool queries matched independent gold results; natural lexical reuse matched `1/76`, while controlled typed-template replay matched `75/75` mutated targets. A new family-disjoint frontier replay using 16 actual validated trace artifacts scored `4/20` versus `3/20` no-skill and `3/20` placebo, with one paired win, zero losses, and 19 ties. Governed capsules reject stale authority, wrong scope, expiry, schema drift, unsafe interpolation, and parameter errors. | Validation gate and controlled parameter mechanics supported; one small transfer signal is directionally positive but not promotion evidence | Repeated natural intents, compatible templates, changed-system semantic outcomes, real tool contracts, deletion/failover, more seeds/models, and user utility. |
...
| Embedding-vs-model insight mining | Action embeddings improved candidate Recall@20 by `+.123` on MATM, but on 76 outcome-backed BIRD artifacts dense, lexical, identifier, and hybrid retrieval all gave `0/76` top-1 executable matches and no template hits. Luna tied lexical on nine reranker queries at `11.569s` mean per call. | Candidate-generation mechanics only; no dense promotion | Common human/SME insight labels, fixed cost budgets, adversarial NILs, repeated intents, and artifact replay. |
...
| Skill improvement | One-shot trace-mined procedure was `8/40`, equal to no-skill and slower. The new validated-artifact library arm reached `4/20` versus `3/20` no-skill and `3/20` placebo, but only one paired win and no losses. These are verified checkpoints, not a universal disproof of sequential skill learning. | Keep prose procedure quarantined; retain validated artifacts only as scope-bound experimental candidates | Sequential repair/clarification consumers, compatible repeated intents, changed-system outcomes, more seeds/models, and independent labels. |
| Cross-user insights | Trace Commons prompt retrieval reached `13/13` and durable-identifier retrieval `12/13` same-project top-1 on a repeated-workstream proxy; event structure was `1/13`. The corrected local Codex archive importer exposes 622 sessions and 47,122 prompt episodes; lexical friction markers had only `.79%` precision against a structured process-exit proxy, and no stable enterprise labels or independent outcomes exist. | Workstream/friction-signal proxies only; cross-user insight unproven | Consent-stable identities, principal/time-disjoint task labels, human capability labels, same-surface negatives, and negative-transfer gates. |
