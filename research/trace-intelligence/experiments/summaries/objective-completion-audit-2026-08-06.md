...
| Corporate concept/alias discovery | Modernized termhood produced 3,000 candidates; within-schema recall `.358`, held-out transfer `.015`. Acronym probe passed `8/8` synthetic cases with ambiguity abstention. The deterministic train-only alias-enrichment reproduction was identical to the prior receipt: lexical MRR `.734885`, support-1 aliases `.734885`, support-2 aliases `.727542`, with only `.065385` target coverage and 506 ambiguous surfaces. The strengthened cohort contract now requires five examples of each semantic mutation stratum before promotion. | Candidate mining only; frequency-derived aliases are not yet useful enough for promotion | Obtain independently reviewed aliases, temporal replacements, NILs, same-scope wrong-system labels, all required mutation strata, and downstream utility. |
| Hard-negative mining | Scope-aware identifier retrieval reduced collision-before-target from `14.43%` to `.20%`; identifier-aware reranker reached MRR `.737` and collision rate `0.0` on a held-out public proxy. The full 500-question EnterpriseRAG source-filter ceiling improved MRR `.511064 -> .593602` and eliminated wrong-source extras, but left `8.963830` same-source non-targets per top-10 list and returned candidates for all 30 targetless questions. | Useful bounded method; source scope is necessary but not sufficient | Larger expert-labeled same-surface/wrong-system, temporal, NIL, and result-preserving hard-negative families with changed-system replay. |
