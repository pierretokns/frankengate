...
| Family-disjoint BIRD composition | Two replays: library `8/40`; no-skill `6/40`; placebo `6/40`; one stable library win and zero stable losses against each control | The aggregate composition signal is stable across runs, but task-level headroom is sparse. | Statistical lift, changed schemas, irrelevant-library abstention, or enterprise transfer. |
