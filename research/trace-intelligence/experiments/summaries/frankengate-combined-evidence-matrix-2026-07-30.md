...
| `L6_procedural_replay` | `governed_component_isolation_partial_pass` | 61/61 DTO, broker, ledger, evaluator, stage, resolver, process, and OCI-contract tests pass; one real Linux OCI boundary and one real PostgreSQL role/snapshot slice pass, but the final minimal-image 27-gate run and causal skill benefit do not |
...
- The NL2SQL capability checkpoint passes 61/61 component tests. Its combined test executes one candidate, exposes an empty model preview, seals the full three-row typed result, submits without another database call, and evaluates the full blob while the execution counter remains one. A real Linux `runc` pass exercised 21 boundary/protocol gates and caught two profile defects invisible to unit tests: missing safe `fstatfs`, and host-global `RLIMIT_NPROC` coupling to another workload sharing UID 65532. A real PostgreSQL 16 slice passed three candidate and three evaluator-only gold executions through different constrained identities, write denial, database snapshot stability, and cleanup. This still does not substitute for a minimal solver image, episode-specific identities, OS resolver peer credentials, independent server execution receipts, crash recovery, signed OTel, or the complete 27-gate proof.
- The NL2SQL enterprise-domain assessment admits 39 pinned dataset manifests and selects one deliberately small next study: mine one generalized procedure from evidence-only WMH BIRD families, then compare no artifact, length-matched placebo, and that frozen procedure on paired family-disjoint Defog tasks. BIRD-Interact, FINCH, MAGIC, PT-BR, Spider2, pooled methods, custom embeddings, and additional storage are follow-ons only after held-out causal value. None of the public SQL corpora supports employee or team skill inference; that claim requires a separate prospective, consented within-user pilot.
- The newly admitted DiscoPosse OTel shard supports real cross-schema projection and selector testing, but the selector is workload-sensitive: on an `appworld` shard, error/tool signals reached 1.00 precision at a 20% review budget, while a `browsecompplus` shard had zero error-status positives and every arm scored zero. A bounded mixed dataset-server sample tied signals with trace length at 1.00 precision. These are failure-triage proxies only, not human-informative or enterprise outcomes.
