...
| Do the modernized term/acronym ideas recur across a real multi-project history? | On 432 sessions across 65 project directories, 777/2,249 top-term hashes crossed at least two projects and 36/170 valid acronym hashes crossed a project boundary; no candidate appeared in all projects. | Keep project/time-scoped candidate mining; do not build a global alias table from frequency. | Reviewed concepts, shared-boilerplate controls, temporal/wrong-system negatives, and downstream replay/user outcomes. |
...
| Can project-held-out lexical adaptation help on real user histories? | Full Peter DataClaw cohort combined prompt+tool MRR `.769341→.854452`; Vaynelee remained at `.978495` ceiling; Peter tool-only MRR declined `.675115→.665805`. | Scoped adaptation is promising when the baseline has headroom; no universal custom embedding promotion. | Expert project/task labels, time drift, hard negatives, replayable artifacts, and next-task outcomes. |
