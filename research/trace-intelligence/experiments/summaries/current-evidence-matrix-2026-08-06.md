...
| Does validated subplan composition transfer across families? | BIRD family-disjoint library replay: `4/20` exact versus `3/20` for both no-skill and placebo; one paired library win and zero losses against each control. | Keep subplans as a separate experimental artifact granularity; the signal is encouraging but underpowered. | More families/seeds, irrelevant-library NILs, changed-schema replay, and enterprise outcomes. |
