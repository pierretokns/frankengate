...
1 durably evidenced natural cases; observed
typed component failures: langmem:AttributeError, graphiti:ValidationError, langmem:AttributeError. No proxy result was substituted.
...
The execution rows record model-specific natural failures: LangMem completed
one case before two `AttributeError` failures, while Graphiti reached one case
and returned a `ValidationError`. No proxy result was substituted. A natural
LangMem candidate was not produced in the one completed case.
...
reported. In the LangMem arm, the explicit zero observations above
...
3. The selected Ollama model was used through the public OpenAI-compatible
   client surfaces with `extra_body.think=false`. Model-specific structured-
   output and validation failures remain typed execution rows, not zero scores.
...
| Local inference | Ollama OpenAI-compatible endpoint, `llama3.2:latest`, reasoning disabled |
...
`llama3.2:latest` for extraction, Nomic Embed Text at 768 dimensions, temperature zero,
