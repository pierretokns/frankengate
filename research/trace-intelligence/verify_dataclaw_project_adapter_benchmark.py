...
if __name__ == "__main__":
    ...
    checks = {"schema": result.get("schema_version") == "frankengate-dataclaw-project-adapter-v1", "result_hash": expected == digest(unsigned), "modes": bool(set(modes) & {"prompt", "tool", "combined"}), "folds": all(modes[name]["folds"] > 0 for name in modes), "bounded": all(0 <= modes[name][metric] <= 1 for name in modes for metric in ("baseline_mrr", "adapted_mrr", "baseline_recall_at_1", "adapted_recall_at_1", "baseline_recall_at_5", "adapted_recall_at_5")), "raw_absent": result.get("source", {}).get("raw_content_committed") is False, "claim_boundary": result.get("claim_boundary", {}).get("promotion_authorized") is False}
