		...
		<main className="bg-background text-foreground flex min-h-screen items-center justify-center p-6">
			<section className="bg-card w-full max-w-xl rounded-sm border p-8 text-center shadow-sm">
				...
				<h1 className="text-xl font-semibold tracking-tight">
					{isComplete ? `${PRODUCT_NAME} Agent sign-in complete` : `${PRODUCT_NAME} Agent sign-in`}
				</h1>
				<p className="text-muted-foreground mt-2 text-sm">
					{isComplete ? "You can close this window and return to the agent." : `Sign-in status: ${status}`}
				</p>
			</section>
		</main>
	);
