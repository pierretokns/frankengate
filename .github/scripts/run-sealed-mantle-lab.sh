...
jq -n --slurpfile ca "$build/claude-attestations.json" --slurpfile xa "$build/codex-attestations.json" --arg casha "$(sha256sum "$build/claude-attestations.json" | cut -d' ' -f1)" --arg xasha "$(sha256sum "$build/codex-attestations.json" | cut -d' ' -f1)" --arg run "$run_id" --arg source "$source_hash" --arg bifrost "$bifrost_ref" --arg codex "$codex_ref" --arg claude "$claude_ref" --arg sentinel "$sentinel_ref" --arg cv "$codex_version" --arg av "$claude_version" '{schema:"sealed-lab-runtime-lock/v1",run_id:$run,source_lock_sha256:$source,images:[{id:"bifrost",reference:$bifrost,platforms:["linux/amd64","linux/arm64"],source:("git:"+env.GITHUB_SHA)},{id:"claude-runner",reference:$claude,platforms:["linux/amd64","linux/arm64"],source:("lock:"+$av),client_version:$av,child_digests:($ca[0]|map({platform,sha256:.child_digest})),attestation_sha256:$casha},{id:"codex-runner",reference:$codex,platforms:["linux/amd64","linux/arm64"],source:("lock:"+$cv),client_version:$cv,child_digests:($xa[0]|map({platform,sha256:.child_digest})),attestation_sha256:$xasha},{id:"egress-sentinel",reference:$sentinel,platforms:["linux/amd64","linux/arm64"],source:("git:"+env.GITHUB_SHA)}]}' >"$artifacts/runtime-lock.json"
