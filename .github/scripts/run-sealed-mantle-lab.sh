...
jq --slurpfile ca "$build/claude-attestations.json" --slurpfile xa "$build/codex-attestations.json" '.images |= map(if .id=="claude-runner" then .attestations=$ca[0] elif .id=="codex-runner" then .attestations=$xa[0] else . end)' "$artifacts/runtime-lock.json" >"$artifacts/runtime-lock.tmp" && mv "$artifacts/runtime-lock.tmp" "$artifacts/runtime-lock.json"
