...
cleanup() {
  ...
  for file in "$artifacts"/*; do
    ...
    case "$(basename "$file")" in diagnostics.json|runtime-lock.json|lifecycle.json|compose.log|compose-ps.txt) ;; *) rm -f "$file"; bounds=1; continue ;; esac
    size="$(stat -c %s "$file")"; count=$((count+1)); total=$((total+size))
    if (( size > 4194304 )); then rm -f "$file"; bounds=1; fi
  done
  if (( count > 5 || total > 8388608 )); then bounds=1; fi
