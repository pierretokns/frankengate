---
title: "Run FrankenGate Benchmarks"
description: "Step-by-step guide to measure FrankenGate overhead and provider workload behavior in your own environment."
icon: "stopwatch"
---

## Overview

Want to measure FrankenGate in your own environment? Start with the fork-owned
scripts in `scripts/`. They run without live provider credentials and are the
authoritative path for gateway-overhead regression checks:

```bash
N=1000 MAX_P50_OVERHEAD_MS=5 MAX_P95_OVERHEAD_MS=10 \
  bash scripts/benchmark-gateway-overhead-local.sh
```

The local fixture compares a direct response with a gateway-shaped path and
reports p50/p95 overhead. Use a real provider or a separately managed load
generator only after this deterministic baseline is green.

The historical [upstream Bifrost benchmarking repository](https://github.com/maximhq/bifrost-benchmarking)
is retained below solely as a compatibility/reference tool. It is not a
FrankenGate release, install source, support route, or performance claim.

**What the workload harness can test:**
...
The historical repository also ships two companion tools:
...
> **Reference notice:** upstream tools and links below retain their original
> names for compatibility. They are not maintained or published by FrankenGate.

---

## Historical upstream workload workflow

The remaining instructions document the upstream workload harness for teams
that need compatibility with it. They do not publish FrankenGate install or
support commands; use the local script above for fork-owned regression checks.
