...
<Info>
  **Enterprise Feature**: Adaptive Load Balancing is available in Bifrost
  Enterprise routing features are tracked in the FrankenGate roadmap and are
  enabled through the fork's governance configuration.
  it.
</Info>
...
5. **failover handling**: Skip keys with zero weight (TPM hits, repeated failures)
...
| **failover handlings**       | Failing routes automatically removed from rotation                  |
