...
export const ModelsCatalog = () => {
  ...
  const [pricingAttribution, setPricingAttribution] = useState(null)

  useEffect(() => {
    async function fetchModels () {
      try {
        const paths = ['/data/pricing/latest.json', 'data/pricing/latest.json', 'https://pierretokns.github.io/frankengate/data/pricing/latest-upstream.json']
        let data
        let lastError
        ...
        if (!data) throw lastError || new Error('Pricing catalog unavailable')
        setPricingAttribution(data.models ? { source: data.source, retrievedAt: data.retrieved_at } : null)
        const catalog = data.models || data
        
        // Convert object format {modelName: {config}} to array format
        if (catalog && typeof catalog === 'object' && !Array.isArray(catalog)) {
          const modelsArray = Object.entries(catalog).map(([modelName, config]) => ({
            model: modelName,
            ...config
          }))
  ...
  return (
    <div className="w-full not-prose" style={{ display: 'flex', flexDirection: 'column' }}>
      {pricingAttribution && (
        <p className="mb-3 text-xs text-zinc-600 dark:text-zinc-400" data-testid="pricing-attribution">
          Pricing source: {pricingAttribution.source || 'upstream datasheet'}
          {pricingAttribution.retrievedAt ? ` · refreshed ${pricingAttribution.retrievedAt}` : ''}
        </p>
      )}
