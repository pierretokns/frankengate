...
| GEPA / SkillOpt / Trace2Skill / EvoSkill | Bounded candidate search and hierarchical skill updates | **External SkillOpt evidence + local mock plumbing; natural Frankengate intervention remains unrun.** Current SkillOpt source `7da46ae…` and v0.2.0 `51d0a4d…` were inspected; its mock Sleep experiment moved held-out score 0.3333→1.0 and blocked a harmful edit. External reports claim 52/52 best-or-tied cells and Sleep SearchQA gains, but do not provide our raw natural-trace effect | Q5, Q6 | Common family-disjoint no-skill/placebo/expert/mined/SkillGen/SkillOpt/RHO arms with paired repairs/regressions and independent test outcomes |
| SkillGen | Contrast successful and failed trajectories into one auditable skill; verify paired same-instance repairs and regressions | **Reviewed.** The intervention design directly matches Frankengate’s missing causal gate; no local run | Q5, Q6 | Run paired natural traces with independent outcome verifier and no hidden-test access |
| Retrospective Harness Optimization (RHO) | Self-supervised coreset selection and harness updates from past trajectories | **Reviewed/external result.** Reported SWE-Bench Pro 59%→78% after one round; no local reproduction and self-preference is not an independent oracle | Q5, Q6 | Compare RHO candidate search against SkillOpt/SkillGen under the same frozen test and independent evaluator |
| SigLeak | Infer procedural skills from behavior-only traces | **Reviewed/security result.** Reported +6.88 points while reconstructing skills; demonstrates that cross-user trace sharing can leak proprietary procedures | Q1, Q4, Q6 | Add skill-leakage probes, egress scans, consent, and scoped projections before any cross-user learning |
...
| H5 governed PostgreSQL live reruns | Disposable `colima` PostgreSQL 16.12/pgvector 0.8.1; transaction-scoped forced-RLS assertions plus independent concurrency schedules | **Direct plus corrected contract race.** Original H5 reproduced the active-exposure metadata gap. New row-locking procedures passed rollback-only assertions and a two-session race: exposure commits first, withdrawal waits, then ends exposure and appends one event; zero active exposures remain | Continuous revocation under REPEATABLE READ and Aurora/RDS Proxy behavior remain open; direct legacy table writes still require migration enforcement |
