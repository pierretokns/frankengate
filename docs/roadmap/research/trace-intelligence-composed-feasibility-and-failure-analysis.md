...
| FrankenSearch | Tantivy-style lexical search, dense fusion, reranking, compact local indexes, progressive results. | **Research-only/default-off sidecar.** When deployed inside an equivalent tenant/purpose/RLS boundary it may receive authorized full-fidelity chunks after credential stripping; otherwise it receives a destination-transformed projection. It also receives tombstones, never participates in gateway readiness, and must pass auth/deletion/license/failure-isolation gates. |
