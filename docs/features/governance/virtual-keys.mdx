...
## Controlled-overdraft notifications

Controlled overdraft settlement can emit an `OverdraftEvent` through the
governance notifier boundary. Delivery is asynchronous: notification failures
or a full bounded queue are recorded without holding the inference request
open. The event contains the reservation ID, reserved amount, observed amount,
excess, approval decision, and policy reason; it does not contain provider
credentials, prompt content, or virtual-key secrets.

The pluggable `WebhookOverdraftNotifier` adapter is suitable for an internal
alert service. When wired by a deployment, it sends JSON with a stable
`Idempotency-Key` derived from the reservation ID and an
`X-FrankenGate-Signature` HMAC-SHA256 header. Transient failures are retried
with bounded exponential backoff; client errors are not retried.

SNS and SES are also supported through the same asynchronous boundary. The
`sns` channel reads `topic_arn` and optional `subject`/`region`; the `email` channel
reads `from`, comma-separated `recipients`, and optional `subject`/`region`. FrankenGate
uses the AWS SDK default credential and region chain, so IAM policies and SES
identity verification remain deployment responsibilities. If AWS credentials
or channel configuration are unavailable, delivery fails closed and inference
continues.

Use Prometheus/Alertmanager (or OTEL metrics routed to a collector) for
service-level alerts such as delivery failures, queue drops, and outbox
growth. SNS/SES are user-facing notifications, not a replacement for SLO
monitoring. All delivery remains asynchronous and bounded; it never holds the
inference request open.
