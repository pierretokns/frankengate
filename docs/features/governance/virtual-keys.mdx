...
The built-in webhook adapter is suitable for an internal alert service. It
sends JSON with a stable `Idempotency-Key` derived from the reservation ID and,
when configured, an `X-FrankenGate-Signature` HMAC-SHA256 header. Transient
failures are retried with bounded exponential backoff; client errors are not
retried. SNS/SQS and in-product delivery should be implemented as consumers of
the same event contract, and are optional integrations rather than mandatory
