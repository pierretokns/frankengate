...
Use OTEL when you want provider-neutral traces and metrics in Grafana,
Datadog, Honeycomb, New Relic, or a self-hosted collector. Evaluation,
experimentation, and governance consumers should read the fork's redacted
replay and analytics contracts rather than relying on a vendor-specific plugin.
...
Replay capture is opt-in and runs asynchronously after the response is
written. Set `FRANKENGATE_REPLAY_S3_BUCKET` to use the shared S3-compatible
object-store path (the pod's IAM role supplies credentials); optionally set
`FRANKENGATE_REPLAY_S3_PREFIX` and `AWS_REGION`. Without that variable, set
`FRANKENGATE_REPLAY_DIR` to use tenant-partitioned JSONL on a mounted volume.
Both paths redact content and identity headers before persistence and reject
records without a tenant. S3/object-store failure is surfaced to the
observability metrics and never blocks inference.
...
- **[Analytics and replay contract](/roadmap/architecture/otel-storage-plane)** - Governed trace storage and replay
