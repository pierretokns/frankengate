...
fn handle_connection(
    ...
    let (status, content_type, body) = match path {
        ...
        "/v1/experiments" if method == "GET" => {
            ...
            match (database.as_ref(), tenant) {
                (Some(pool), Some(tenant)) => {
                    let experiments = tokio::runtime::Runtime::new().ok().and_then(|runtime| {
                        runtime
                            .block_on(frankengate_analytics_control::db::list_experiments(
                                pool, tenant, limit,
                            ))
                            .ok()
                    });
                    match experiments {
                        Some(experiments) => match serde_json::to_string(&experiments) {
                            Ok(body) => ("200 OK", "application/json", body),
                            Err(_) => (
                                "500 Internal Server Error",
                                "text/plain",
                                "failed to encode experiments\n".to_string(),
                            ),
                        },
                        None => (
                            "503 Service Unavailable",
        ...
        "/v1/evaluations" if method == "GET" => {
            ...
            match (database.as_ref(), tenant, run_id) {
                (Some(pool), Some(tenant), Some(run_id)) => {
                    let evaluations = tokio::runtime::Runtime::new().ok().and_then(|runtime| {
                        runtime
                            .block_on(frankengate_analytics_control::db::list_evaluations(
                                pool, tenant, run_id, limit,
                            ))
                            .ok()
                    });
                    match evaluations {
                        Some(evaluations) => match serde_json::to_string(&evaluations) {
                            Ok(body) => ("200 OK", "application/json", body),
                            Err(_) => (
                                "500 Internal Server Error",
                                "text/plain",
                                "failed to encode evaluations\n".to_string(),
                            ),
                        },
                        None => (
                            "503 Service Unavailable",
        ...
        "/v1/attempts" if method == "GET" => {
            ...
            match (database.as_ref(), tenant, run_id) {
                (Some(pool), Some(tenant), Some(run_id)) => {
                    let attempts = tokio::runtime::Runtime::new().ok().and_then(|runtime| {
                        runtime
                            .block_on(frankengate_analytics_control::db::list_attempts(
                                pool, tenant, run_id, limit,
                            ))
                            .ok()
                    });
                    match attempts {
                        Some(attempts) => match serde_json::to_string(&attempts) {
                            Ok(body) => ("200 OK", "application/json", body),
                            Err(_) => (
                                "500 Internal Server Error",
                                "text/plain",
                                "failed to encode attempts\n".to_string(),
                            ),
                        },
                        None => (
                            "503 Service Unavailable",
