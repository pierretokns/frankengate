...
`/jobs/complete?id=<id>&worker=<worker>` for owner-scoped transitions,
`/jobs/renew?id=<id>&worker=<worker>` for heartbeats, and
...
`/contract` for a machine-readable feature/configuration inventory, and
